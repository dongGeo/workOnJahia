...
<html lang="en"> <!--<![endif]-->
  ...
  <head>
	...
	<template:module node="${renderContext.site}" view="displayMeta" editable="false" />
	
    <!-- Favicon -->
    <c:choose>
      <c:when test="${jcr:isNodeType(renderContext.site, 'eprsmix:favicon')}">
        <template:module path="${renderContext.site.path}" view="favicon"  editable="false" />
      </c:when>
      <c:otherwise>
	...
	<c:choose>
		<c:when test="${((renderContext.editModeConfigName eq 'studiomode') or(renderContext.editModeConfigName eq 'studiovisualmode'))}">
	        <c:set var="nodename" value="${renderContext.mainResource.node.properties['j:nodename'].string}" />
		</c:when>
		<c:otherwise>
			<c:set var="nodename" value="${renderContext.mainResource.node.properties['j:templateName'].string}"/>
			<c:if test="${empty nodename }"><%-- when the mainResource is detail --%>
			  <c:set value="${jcr:getParentOfType(renderContext.mainResource.node, 'jnt:page')}" var="mainResourceParent"/>
			  <c:set var="nodename" value="${mainResourceParent.properties['j:templateName'].string}"/>
			</c:if>		
		</c:otherwise>
	</c:choose> 
  </head>
  <body>
    ...
    <header>
      <section class="header-section" role="toolbar" >
        <div class="header-section__base">          
          ...
          <template:area path="header-section-1"/> 
          <c:if test="${nodename != 'home' }" >
          	<template:area path="header-section-search"/>
          </c:if> 
          <template:area path="header-section-2"/> 
      </div>        
     </section>
    ...
    <main class="homepage"><%-- class variable --%>
      ...
      <c:if test="${nodename == 'home' || nodename == 'base'  }" >
